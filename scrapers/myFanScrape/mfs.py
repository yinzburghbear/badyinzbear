...
# IMAGES    ########################################################################################
def lookup_image_fragment(fragment_name):
    """
    Attempt to match image fragment (no extension) across known user_data.db files.
    """
    if sys.version_info >= (3, 9):
        fragment_base = fragment_name.removesuffix("_source")
    else:
        fragment_base = fragment_name[:-7] if fragment_name.endswith("_source") else fragment_name

    fragment_base = os.path.splitext(fragment_base)[0].lower()

    search_path = Path(META_BASE_PATH or ".").resolve()
    log.info(f"Searching for image fragment: {fragment_base} in path: {search_path}")

    try:
        db_files = list(search_path.rglob("user_data.db"))
        if not db_files:
            log.error("No user_data.db files found")
    except Exception as e:
        log.error(f"Error searching for DBs: {e}")
        print(json.dumps({"error": "DB search failed"}))
        sys.exit()

    for db_file in db_files:
        log.info(f"Checking {db_file}")
        ...
        try:
            c.execute("""
                ...
                FROM medias
                WHERE LOWER(REPLACE(REPLACE(REPLACE(REPLACE(filename, '_source', ''), '.jpg', ''), '.jpeg', ''), '.png', '')) = ?
            ...
            if row:
                log.info(f"Found match: {row[0]} in {db_file}")
                filename, post_id = row
                username = db_file.parent.name
                network = "Fansly" if "Fansly" in str(db_file) else "OnlyFans"
                ...
                if not meta_row:
                    log.error("Post metadata not found.")
                    break

                result = process_row(meta_row, username, network, filename)
            ...
            log.error(f"Error processing {db_file}: {e}")
            continue

    log.error(f"No match found for fragment: {fragment_base}")
    print(json.dumps({"error": f"Fragment '{fragment_base}' not found"}))  # prevent EOF
    sys.exit()
...
def main():
    """
    Execute scene, gallery, or image lookup and print the result as JSON to stdout
    """
    ...
    elif sys.argv[1] == "queryImage":
        title = fragment.get("title", "")
        if not title:
            log.error("Missing 'title' in fragment")
            print("null")
            sys.exit()

        media = lookup_image_fragment(title)
    ...
    # shared logic for scene/gallery only
    ...
    log.debug(f"Script runtime: after media lookup: {time.monotonic() - start_time} seconds")
    print(json.dumps(media))
    log.debug(f"Script runtime: total runtime: {time.monotonic() - start_time} seconds")
    sys.exit()
    log.error(f"No match found for image fragment: {fragment_base}")
    print(json.dumps({"error": "not found"}))  # prevent EOF
    sys.exit()
if __name__ == "__main__":
    main()
    
# yinzburghbear's Fanscrape version created by yinzburghbear and chatgpt based on the original Fanscrape pyscraper.