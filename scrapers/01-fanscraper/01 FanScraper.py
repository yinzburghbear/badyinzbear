...
def lookup_image_fragment(image_id: int):
    ...
    if studio_name.endswith(" (OnlyFans)"):
        network = "OnlyFans"
    elif studio_name.endswith(" (Fansly)"):
        network = "Fansly"
    else:
        log.error(
    ...
    username = studio_name.removesuffix(f" ({network})")
    log.debug(
    ...
    db_path = get_metadata_db(Path(folder_path), username, network)
    if not db_path:
        log.error(f"No database found for image_id={image_id}, username={username}")
        return {}
    log.debug(f"Resolved DB path for image_id={image_id}: {db_path}")
